...
const mockStoriesData = [
  ...
  },
  {
    ...
    stats: [
      ...
      { label: "Career transition", value: "Success", icon: Trophy }
    ]
  }
];
...
const SuccessStories = () => {
  ...
  const { data: apiStories = [], isLoading } = useGetSuccessStories(activeCategory);

  // Map backend stories or use mock if none return yet
  const storiesData = apiStories.length > 0
    ? apiStories.map(s => ({
        id: s._id,
        name: s.name,
        role: s.jobTitle,
        category: s.industry || s.course || "Career Story",
        avatar: s.avatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=200&auto=format&fit=crop",
        quote: s.story,
        audioUrl: s.audioUrl,
        duration: 165,
        stats: s.stats && s.stats.length > 0 ? s.stats.map(st => ({ ...st, icon: Briefcase })) : [
          { label: "Course", value: s.course || "General", icon: Briefcase }
        ]
      }))
    : mockStoriesData;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredStories.slice(0, visibleCount).map((story) => (
                <SuccessStoryCard
                  ...
                  isMuted={mutedStates[story.id] || false}
